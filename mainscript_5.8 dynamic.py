...
def filesave(name,matrix,printpath):
    if printpath == True:
        print('Saving: "savedata/'+str(name)+'.npy"')
    np.save("savedata/"+str(name),matrix)
...
def initialize_layers():
    ...
    if reset_network == False:
        ...
        for name in name_list:
            try:
                globals()[name] = np.load("savedata/"+name+".npy")
                loaded +=1
                found.append(name)
                print('Loaded: "savedata/'+name+'.npy"')
            except:
                print('No array saved at location: "savedata/'+name+'.npy"')
                missing.append(name)
...
def backpropagation(targetlist):
